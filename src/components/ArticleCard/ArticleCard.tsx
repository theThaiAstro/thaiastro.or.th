	...
	<div className={cx('w-full', props.classNames)}>
		<div>
			<div className={cx('relative aspect-video h-full w-full')}>
				<Image alt={props.article.title} src={getAsset(props.article.thumbnail?.id)} layout="fill" objectFit="cover" unoptimized={true} />
			</div>
		</div>
		<div className="mt-6">
			<Heading className="font-['IBM_Plex_Sans_Thai'] text-3xl font-semibold">{props.article.title}</Heading>
			<div className={cx('mt-2 overflow-hidden')}>{trimSentence(props.article.excerpt ?? props.article.content, true)}</div>
		</div>
	</div>
);
...
export const HeroArticleCard: React.FC<Props> = (props: Props) => (
	<div className={cx('', 'md:flex md:h-80')}>
		...
		<div className={cx('mt-6', 'flex-grow md:mt-0 md:ml-8')}>
			...
			<div className={cx('', 'mt-2 overflow-hidden', 'md:mt-4')}>{trimSentence(props.article.excerpt ?? props.article.content, true)}</div>
		</div>
	</div>
);
